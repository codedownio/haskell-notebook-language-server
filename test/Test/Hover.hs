...
spec = describe "Hover" $ do
  describe "fixes up document references" $ do
    let (_projected, transformer) = project (transformerParams GHC.Paths.libdir) (listToDoc hoverLines)

    it "Basic case" $ do
      fixupDocumentReferences' GHC.Paths.libdir (mkDocRegex "main.ipynb") transformer "Defined at: main.ipynb:3:1"
    ...
    it "Works with a long path with spaces and stuff" $ do
      ...
      fixupDocumentReferences' GHC.Paths.libdir regex transformer [i|Defined at: #{longPath}:3:1|]
        >>= (`shouldBe` [i|Defined at: #{longPath}:1:1|])

    it "Works with weird PCRE characters in the file name" $ do
      fixupDocumentReferences' GHC.Paths.libdir (mkDocRegex "ma$in.i?pynb") transformer "Defined at: ma$in.i?pynb:3:1"
