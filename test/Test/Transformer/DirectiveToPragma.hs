...
spec = describe "DirectiveToPragma" $ do
  it "Converts :set -XFoo directive to LANGUAGE pragma" $ do
    (ls, dp@(DirectiveToPragma affectedLines)) <- project (DTPParams GHC.Paths.libdir) (listToDoc ["foo = 42", ":set -XFoo"])
  ...
  it "Converts :set -XFoo -XBar directive to LANGUAGE pragmas" $ do
    (ls, dp@(DirectiveToPragma affectedLines)) <- project (DTPParams GHC.Paths.libdir) (listToDoc ["foo = 42", ":set -XFoo -XBar"])
  ...
  it "Does a simple single line change" $ do
    ...
    (_projectedBefore, dp) <- project @DirectiveToPragma (DTPParams GHC.Paths.libdir) doc
    let before = doc
    (changes', _transformer') <- handleDiffMulti (DTPParams GHC.Paths.libdir) before changes dp
