...
spec = describe "ImportSifter" $ do
  it "projects and transforms a single import" $ do
    let (ls, sifter@(ImportSifter indices)) = project () ["putStrLn 42", "import Bar"]
  ...
  it "projects and transforms two imports" $ do
    let (ls, sifter@(ImportSifter indices)) = project () ["putStrLn 42", "import Bar", "foo = 42", "import Foo", "bar = 24"]
