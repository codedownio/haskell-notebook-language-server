...
spec = describe "PragmaSifter" $ do
  it "projects and transforms a single pragma" $ do
    let (ls, sifter@(PragmaSifter indices)) = project () (listToDoc ["putStrLn 42", "{-# LANGUAGE Foo #-}"])
  ...
  it "projects and transforms two pragmas" $ do
    let (ls, sifter@(PragmaSifter indices)) = project () (listToDoc ["putStrLn 42", "{-# LANGUAGE Bar #-}", "foo = 42", "{-# LANGUAGE Foo #-}", "bar = 24"])
